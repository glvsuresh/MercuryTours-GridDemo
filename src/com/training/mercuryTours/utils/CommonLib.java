...
public class CommonLib {
	...
	public static void openBrowser(String sBrowserName) {
		...
		try {
			...
			case "INTERNET EXPLORER": {
				...
				oCapab=DesiredCapabilities.internetExplorer();
				//oCapab.setBrowserName("iexplore");
				//oCapab.setPlatform(Platform.WINDOWS);
				oBrowser=new RemoteWebDriver(new URL("http://localhost:4444/wd/hub"), oCapab);
			...
			case "CHROME": {
				...
				oCapab = DesiredCapabilities.chrome();
				oCapab.setBrowserName("chrome");
				oCapab.setPlatform(Platform.WINDOWS);
	            oBrowser=new RemoteWebDriver(new URL("http://localhost:4444/wd/hub"), oCapab);
				break;
			}
			...
			case "FIREFOX": {
				...
				oCapab = DesiredCapabilities.firefox();
				//oCapab.setBrowserName("firefox");
				//oCapab.setPlatform(Platform.WINDOWS);
	            oBrowser = new RemoteWebDriver(new URL("http://localhost:4444/wd/hub"), oCapab);
				break;
			}
	...
	public static void navigateToURL(String sURL) {
		try {
			...
			Reporter.log("Error occurred while navigating " + sURL + " due to "
					+ t.getMessage());
		}
	...
	public static void takeScreenshot(String sFileName) {
		try {
			...
			FileUtils.copyFile(ScrFile, new File(Constants.sReportPath
					+ sFileName));
		} catch (Throwable t) {
	...
	public static void selectValFromListbox(WebElement oElem, String sVal) {
		try {
			...
			Reporter.log("Failed to select " + sVal
					+ " value from list box due to " + t.getMessage());
			Assert.fail("Failed to select " + sVal
					+ " value from list box due to " + t.getMessage());
		}
	}
	...
	public static void enterText(By oBy, String sVal) {
		try {
			...
			Reporter.log("Failed to Enter " + sVal
					+ " value into textbox due to " + t.getMessage());
			Assert.fail("Failed to Enter " + sVal
					+ " value into textbox due to " + t.getMessage());
		}
	}
	...
	public static void clickOnElemByusingName(By oBy, String sVal) {
		try {
			...
			for (WebElement oElem : oElems) {
				String sElemName = oElem.getAttribute("value")
						.replace("$", " ");
			...
			System.out.println(t.getMessage());
			Reporter.log("Failed to click on " + sVal + " due to "
					+ t.getMessage());
			Assert.fail("Failed to click on " + sVal + " due to "
					+ t.getMessage());
		}
